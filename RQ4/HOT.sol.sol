...
contract HoloToken is Ownable {string old_name;string old_symbol;uint8 old_decimals;uint256 old_totalSupply;mapping(address => uint256) old_balances;mapping(address => mapping(address => uint256)) old_allowed;bool old_mintingFinished;address old_destroyer;address old_minter;
  ...
  */
  ...
  function transfer(address _to, uint256 _value) public whenMintingFinished returns (bool) {(bool __v1, bool __v2)=(true,  balances[msg.sender] == old_balances[msg.sender] - _value && balances[_to] == old_balances[_to] + _value);require(_value >= 0);
  ...
  */
  ...
  function transferFrom(address _from, address _to, uint256 _value) public whenMintingFinished returns (bool) {(bool __v1, bool __v2)=(true,  balances[_from] == old_balances[_from] - _value && balances[_to] == old_balances[_to] + _value && allowed[_from][msg.sender] == old_allowed[_from][msg.sender] - _value);require(_value >= 0);
    require(_to != address(0));
  ...
  function approve(address _spender, uint256 _value) public whenMintingFinished returns (bool) {(bool __v1, bool __v2)=(true,  allowed[msg.sender][_spender] == _value);require(_value >= 0);
  ...
  /// ensures(true, minter == _minter)
  function setMinter(address _minter) external onlyOwner {(bool __v1, bool __v2)=(true,  minter == _minter);
    minter = _minter;
  }
  /// ensures(true, totalSupply == old(totalSupply) + _amount && balances[_to] == old(balances[_to]) + _amount)
  function mint(address _to, uint256 _amount) external onlyMinter canMint  returns (bool) {(bool __v1, bool __v2)=(true,  totalSupply == old_totalSupply + _amount && balances[_to] == old_balances[_to] + _amount);require(_amount >= 0);require(totalSupply >= 0);
  ...
  /// ensures(true, mintingFinished)
  function finishMinting() external onlyMinter returns (bool) {(bool __v1, bool __v2)=(true,  mintingFinished);
  ...
  /// ensures(true, destroyer == _destroyer)
  function setDestroyer(address _destroyer) external onlyOwner {(bool __v1, bool __v2)=(true,  destroyer == _destroyer);
    destroyer = _destroyer;
  }
  /// ensures(true, balances[destroyer] == old(balances[destroyer]) - _amount && totalSupply == old(totalSupply) - _amount)
  function burn(uint256 _amount) external onlyDestroyer {(bool __v1, bool __v2)=(true,  balances[destroyer] == old_balances[destroyer] - _amount && totalSupply == old_totalSupply - _amount);require(_amount >= 0);require(totalSupply >= 0);
