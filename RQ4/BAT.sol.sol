...
contract SafeMath {
    ...
    /// ensures(true, r == x -y)
    function safeSubtract(uint256 x, uint256 y) internal returns(uint256 r) {(bool __v1, bool __v2)=(true,  r == x -y);require(x >= 0);require(y >= 0);
      assert(x >= y);
      uint256 z = x - y;
      return z;
    }
...
contract Token {uint256 old_totalSupply;
...
contract StandardToken is Token {mapping(address => uint256) old_balances;mapping(address => mapping(address => uint256)) old_allowed;
    ...
    /// ensures(true, balances[_to] == old(balances[_to]) + _value && balances[_from] == old(balances[_from]) - _value && allowed[_from][msg.sender] == old(allowed[_from][msg.sender]) - _value)
    function transferFrom(address _from, address _to, uint256 _value) returns (bool success) {(bool __v1, bool __v2)=(true,  balances[_to] == old_balances[_to] + _value && balances[_from] == old_balances[_from] - _value && allowed[_from][msg.sender] == old_allowed[_from][msg.sender] - _value);require(_value >= 0);
...
contract BAToken is StandardToken, SafeMath {string old_name;string old_symbol;uint256 old_decimals;string old_version;address old_ethFundDeposit;address old_batFundDeposit;bool old_isFinalized;uint256 old_fundingStartBlock;uint256 old_fundingEndBlock;uint256 old_batFund;uint256 old_tokenExchangeRate;uint256 old_tokenCreationCap;uint256 old_tokenCreationMin;
    ...
    /// ensures(true, totalSupply == old(totalSupply) + msg.value * tokenExchangeRate && balances[msg.sender] == old(balances[msg.sender]) + msg.value * tokenExchangeRate)
    function createTokens() payable external {(bool __v1, bool __v2)=(true,  totalSupply == old_totalSupply + msg.value * tokenExchangeRate && balances[msg.sender] == old_balances[msg.sender] + msg.value * tokenExchangeRate);require(fundingEndBlock >= 0);require(fundingStartBlock >= 0);require(tokenCreationCap >= 0);require(tokenExchangeRate >= 0);require(totalSupply >= 0);
    ...
    /// ensures(true, isFinalized)
    function finalize() external {(bool __v1, bool __v2)=(true,  isFinalized);require(fundingEndBlock >= 0);require(tokenCreationCap >= 0);require(tokenCreationMin >= 0);require(totalSupply >= 0);
      if (isFinalized) throw;
     ...
     /// ensures(true, totalSupply == old(totalSupply) - old(balances[msg.sender]) && balances[msg.sender] == 0)
    function refund() external {(bool __v1, bool __v2)=(true,  totalSupply == old_totalSupply - old_balances[msg.sender] && balances[msg.sender] == 0);require(fundingEndBlock >= 0);require(tokenCreationMin >= 0);require(tokenExchangeRate >= 0);require(totalSupply >= 0);
