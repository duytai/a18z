...
contract BasicToken is ERC20Basic {mapping(address => uint256) old_balances;
  ...
  */
  ...
  function transfer(address _to, uint256 _value) public returns (bool) {(bool __v1, bool __v2)=(true,  balances[msg.sender] == old_balances[msg.sender] - _value && balances[_to] == old_balances[_to] + _value);require(_value >= 0);
...
contract StandardToken is ERC20, BasicToken {mapping(address => mapping(address => uint256)) old_allowed;
   ...
   */
  /// ensures(true, balances[_from] == old(balances[_from]) - _value && balances[_to] == old(balances[_to]) + _value && allowed[_from][msg.sender] == old(allowed[_from][msg.sender]) - _value && r)
  function transferFrom(address _from, address _to, uint256 _value) public returns (bool r) {(bool __v1, bool __v2)=(true,  balances[_from] == old_balances[_from] - _value && balances[_to] == old_balances[_to] + _value && allowed[_from][msg.sender] == old_allowed[_from][msg.sender] - _value && r);require(_value >= 0);
    require(_to != address(0));
  ...
  /// ensures(true, allowed[msg.sender][_spender] == old(allowed[msg.sender][_spender]) - _subtractedValue)
  function decreaseApproval(address _spender, uint _subtractedValue) public returns (bool) {(bool __v1, bool __v2)=(true,  allowed[msg.sender][_spender] == old_allowed[msg.sender][_spender] - _subtractedValue);require(_subtractedValue >= 0);
...
contract Ownable {address old_owner;
   ...
   */
  /// ensures(true, owner == newOwner)
  function transferOwnership(address newOwner) public onlyOwner {(bool __v1, bool __v2)=(true,  owner == newOwner);
...
contract Pausable is Ownable {bool old_pausedPublic;bool old_pausedOwnerAdmin;address old_admin;
   ...
   */
  /// ensures(true, pausedPublic == newPausedPublic && pausedOwnerAdmin == newPausedOwnerAdmin)
  function pause(bool newPausedPublic, bool newPausedOwnerAdmin) onlyOwner public {(bool __v1, bool __v2)=(true,  pausedPublic == newPausedPublic && pausedOwnerAdmin == newPausedOwnerAdmin);
...
contract PausableToken is StandardToken, Pausable {
  /// ensures(true, true)
  function transfer(address _to, uint256 _value) public whenNotPaused returns (bool) {(bool __v1, bool __v2)=(true,  true);require(_value >= 0);
    return super.transfer(_to, _value);
  }
  /// ensures(true, r)
  function transferFrom(address _from, address _to, uint256 _value) public whenNotPaused returns (bool r) {(bool __v1, bool __v2)=(true,  r);require(_value >= 0);
    return super.transferFrom(_from, _to, _value);
  }
  /// ensures(true, true)
  function approve(address _spender, uint256 _value) public whenNotPaused returns (bool) {(bool __v1, bool __v2)=(true,  true);require(_value >= 0);
    return super.approve(_spender, _value);
  }
  /// ensures(true, true)
  function increaseApproval(address _spender, uint _addedValue) public whenNotPaused returns (bool success) {(bool __v1, bool __v2)=(true,  true);require(_addedValue >= 0);
    return super.increaseApproval(_spender, _addedValue);
  }
  /// ensures(true, true)
  function decreaseApproval(address _spender, uint _subtractedValue) public whenNotPaused returns (bool success) {(bool __v1, bool __v2)=(true,  true);require(_subtractedValue >= 0);
...
contract ZilliqaToken is PausableToken {string old_name;string old_symbol;uint8 old_decimals;
    ...
    {(bool __v1, bool __v2)=(true,  true);require(_value >= 0);
        return super.transfer(_to, _value);
    }
    /// ensures(true, r)
    function transferFrom(address _from, address _to, uint _value) validDestination(_to) returns (bool r) 
    {(bool __v1, bool __v2)=(true,  r);require(_value >= 0);
        return super.transferFrom(_from, _to, _value);
    }

    event Burn(address indexed _burner, uint _value);
    /// ensures(true, balances[msg.sender] == old(balances[msg.sender]) - _value && totalSupply == old(totalSupply) - _value)
    function burn(uint _value) returns (bool)
    {(bool __v1, bool __v2)=(true,  balances[msg.sender] == old_balances[msg.sender] - _value && totalSupply == old_totalSupply - _value);require(_value >= 0);require(totalSupply >= 0);
        balances[msg.sender] = balances[msg.sender].sub(_value);
    ...
    /// ensures(true, true)
    function emergencyERC20Drain( ERC20 token, uint amount ) onlyOwner {(bool __v1, bool __v2)=(true,  true);require(amount >= 0);
