...
library SafeMath {
    ...
    */
    /// ensures(b != 0, r == a / b)
    function div(uint256 a, uint256 b) internal pure returns (uint256 r) {(bool __v1, bool __v2)=(b != 0,  r == a / b);require(a >= 0);require(b >= 0);
...
contract NXMToken is IERC20 {mapping(address => uint256) old__balances;mapping(address => mapping(address => uint256)) old__allowed;mapping(address => bool) old_whiteListed;mapping(address => uint256) old_isLockedForMV;uint256 old__totalSupply;string old_name;string old_symbol;uint8 old_decimals;address old_operator;
    ...
    */
    /// ensures(spender != address(0), _allowed[msg.sender][spender] == old(_allowed[msg.sender][spender]) + addedValue)
    ...
    */
    /// ensures(spender != address(0) && _allowed[msg.sender][spender] >= subtractedValue, _allowed[msg.sender][spender] == old(_allowed[msg.sender][spender]) - subtractedValue)
    ...
    */
    /// ensures(whiteListed[to] && from != to && isLockedForMV[from] < now && value <= _balances[from] && value <= _allowed[from][msg.sender], true)
    ...
    * @param value uint256 the amount of tokens to be transferred
    */
    /// ensures(from != to && value <= _balances[from] && value <= _allowed[from][msg.sender], _balances[from] == old(_balances[from]) - value && _balances[to] == old(_balances[to]) + value && _allowed[from][msg.sender] == old(_allowed[from][msg.sender]) - value)
